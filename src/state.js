...
// My Keplr Account Name
const currentAccountNameSelector = selector({
  key: "currentAccountNameSelector",
  get: async ({ get }) => {
    const kState = get(keplrDerviedState);
    if (kState === "loaded") {
      const { offlineSigner } = await AsyncKeplrClient.getInstance();
      const accounts = await offlineSigner.getKey()
      if (Array.isArray(accounts) && accounts[0]) {
        return accounts[0].address;
      } else {
...
const myMintedRocketsState = atom({
  key: "myMintedRocketsState",
  default: [],
});

const myMintedFuelsState = atom({
  key: "myMintedFuelsState",
  default: [],
});
...
const rocketIdState = atom({
  key: "rocketIdState",
  default: 0,
});

const fuelIdState = atom({
  key: "fuelIdState",
  default: 0,
});
